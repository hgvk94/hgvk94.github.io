...
- Run experiment
    ...
    * `cd /popl22-ae/scripts && ./table1.sh` and `cd /popl22-ae/scripts && ./table2.sh` to replicate the results. The first script should take around 6 hours and the second should take around 8 hours to finish. Both scripts will save the output files in `/popl22-ae/rust-horn-experiment` and `/popl22-ae/leon-experiment` directories and also summarize the results in the shell.
...
- Map benchmark directory to a directory inside the docker container
   ...
   ```
   docker run -rm -ti -v /path/to/bench/:/hostBench/ chcModeADTRDF:latest /bin/sh
   ```
